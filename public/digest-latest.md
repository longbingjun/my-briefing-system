# My Briefing · 2026-09-09

12 个活跃信源 -> 4335 条扫描 -> 7 条今日值得看
预计阅读 11 分钟，节省约 180.4 小时。

## 今日导读

- **官方 AI 实验室与公司信号**：官方 AI 实验室与公司信号 本期匹配 1639 篇文章，重点集中在 Agent 工作流、评测/安全、预训练/模型架构。建议先看《Claude's new system prompt really doesn't want to reproduce song lyrics》。
- **AI Agents**：AI Agents 本期匹配 820 篇文章，重点集中在 Agent 工作流、评测/安全、推理训练/测试时计算。建议先看《Using Blender with coding agents on macOS》。
- **推理训练与强化学习**：推理训练与强化学习 本期匹配 541 篇文章，重点集中在 推理训练/测试时计算、预训练/模型架构、产品发布。建议先看《机器人不能停下来等模型：星尘发布 SmoothRL，让在线强化学习跟上大模型的异步推理》。
...
- [97] [Anthropic 升级 Claude Code / Cowork，可后台操控用户 Mac](https://www.ithome.com/0/997/852.htm) · IT之家 · 1 min
...
- [97] [坚决不用行业标准AGENTS.md，Claude Code惹来“封杀令”：Anthropic终于回应了，但开发者更气了](https://www.infoq.cn/article/GuFWNd24Ww5AFlrXxTBo?utm_medium=article) · InfoQ 中文 · 1 min
  点击查看原文>
- [95] [ChatGPT、Grok、Claude、Cursor 集体突发故障](https://www.ithome.com/0/998/195.htm) · IT之家 · 1 min
...
- 官方 AI 实验室与公司信号：本周 344，今日 +67，官方 AI 实验室与公司信号 本期匹配 1639 篇文章，重点集中在 Agent 工作流、评测/安全、预训练/模型架构。建议先看《Claude's new system prompt really doesn't want to reproduce song lyrics》。
- AI Agents：本周 159，今日 +25，AI Agents 本期匹配 820 篇文章，重点集中在 Agent 工作流、评测/安全、推理训练/测试时计算。建议先看《Using Blender with coding agents on macOS》。
- 推理训练与强化学习：本周 100，今日 +7，推理训练与强化学习 本期匹配 541 篇文章，重点集中在 推理训练/测试时计算、预训练/模型架构、产品发布。建议先看《机器人不能停下来等模型：星尘发布 SmoothRL，让在线强化学习跟上大模型的异步推理》。
- 预训练：本周 64，今日 +1，预训练 本期匹配 434 篇文章，重点集中在 预训练/模型架构、推理训练/测试时计算、预训练。建议先看《Qwen3.8-Flash-Next》。
- 编程 Agent：本周 47，今日 +11，编程 Agent 本期匹配 167 篇文章，重点集中在 评测/安全、预训练/模型架构、Agent 工作流。建议先看《Auto mode is now the default in Claude Code for Pro, Max, and Team plans》。
- 后训练与对齐：本周 38，今日 +1，后训练与对齐 本期匹配 306 篇文章，重点集中在 后训练/对齐、预训练/模型架构、推理训练/测试时计算。建议先看《Anthropic 揭示“AI 训练 AI”新方法，比人类研究员成本更低、速度更快》。
- 数据分析与指标体系：本周 259，今日 +24，数据分析与指标体系 本期匹配 1406 篇文章，重点集中在 推理训练/测试时计算、预训练/模型架构、产品发布。建议先看《Understanding ChatGPT Work》。
- AI 评测与安全：本周 243，今日 +12，AI 评测与安全 本期匹配 1503 篇文章，重点集中在 评测/安全、预训练/模型架构、后训练/对齐。建议先看《Auto mode is now the default in Claude Code for Pro, Max, and Team plans》。
- AI 产品化：本周 207，今日 +25，AI 产品化 本期匹配 1290 篇文章，重点集中在 Agent 工作流、评测/安全、推理训练/测试时计算。建议先看《Claude's new system prompt really doesn't want to reproduce song lyrics》。
- AI 基础设施与算力：本周 193，今日 +16，AI 基础设施与算力 本期匹配 1006 篇文章，重点集中在 预训练/模型架构、Agent 工作流、AI 基础设施。建议先看《Qwen 3.8 27B is excellent, but it defaults to wildly overthinking things》。
- 成本管理与 FinOps：本周 147，今日 +17，成本管理与 FinOps 本期匹配 847 篇文章，重点集中在 评测/安全、预训练/模型架构、后训练/对齐。建议先看《On the Navier–Stokes Millennium Prize Problem》。
- AI 改造传统领域：本周 70，今日 +6，AI 改造传统领域 本期匹配 406 篇文章，重点集中在 Agent 工作流、AI 基础设施、评测/安全。建议先看《Claude's new system prompt really doesn't want to reproduce song lyrics》。
